...
class CoverageMerge (object):
    """
    Class CoverageMerge merges multiple coverage report into one
    """
    ...
    def execute_merge(self):
        # get arguments
        logging.basicConfig(
            level=self.loglevel, format='%(levelname)s %(asctime)s: %(message)s', datefmt='%x %X')
        ...
        if not self.xmlfiles:
            ...
            self.xmlfiles = [self.path +
                             filename for filename in self.xmlfiles]
        ...
        if self.filteronly:
            ...
            for xmlfile in self.xmlfiles:
                ...
                self.filter_xml(xml)
                logging.debug(
                    '{1}/{2} filtering: {0}'.format(xmlfile, currfile, totalfiles))
                xml.write(xmlfile + self.filtersuffix,
            ...
            if (totalfiles == 1):
                ...
                xml = ET.parse(xmlfile)
                self.filter_xml(xml)
                xml.write(self.finalxml, encoding="UTF-8",
            ...
            for i in range(totalfiles - 2):
                ...
                logging.debug(
                    '{2}/{3} merging: {0} & {1}'.format(self.finalxml, xmlfile, currfile, totalfiles - 1))
    ...
    def merge_xml(self, xmlfile1, xmlfile2, outputfile):
        ...
        # merge packages
        self.merge(packages1root, packages1, packages2,
    ...
    def prepare_packagefilters(self):
        ...
        for i in range(len(self.packagefilters)):
            self.packagefilters[
                i] = '^' + self.packagefilters[i].replace('.', '\.').replace('*', '.*') + '$'
    ...
    def merge_packages(self, package1, package2):
        ...
        if classes1 or classes2:
            self.merge(package1.find(CLASSES_ROOT), classes1, classes2, [
                       'filename', 'name'], self.merge_classes)
    ...
    def merge_classes(self, class1, class2):
        ...
        if lines1 or lines2:
            self.merge(class1.find(LINES_ROOT), lines1,
                       lines2, 'number', self.merge_lines)

        methods1 = class1.findall(METHODS_LIST)
        methods2 = class2.findall(METHODS_LIST)
        if methods1 or methods2:
            self.merge(class1.find(METHODS_ROOT), methods1,
                       methods2, 'name', self.merge_methods)
    ...
    def merge_methods(self, method1, method2):
        ...
        self.merge(method1.find(LINES_ROOT), lines1,
                   lines2, 'number', self.merge_lines)
